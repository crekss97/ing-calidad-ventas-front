...
export const roleGuard: CanActivateFn = (
  ...
  if (!allowedRoles || allowedRoles.length === 0) {
    console.warn('⚠️ Role Guard: No se especificaron roles requeridos, permitiendo acceso');
    return true;
  }
  ...
  if (hasRequiredRole) {
    console.log(`✅ Role Guard: Usuario con rol ${currentUser.role} tiene acceso`);
    return true;
  }

  // Si no tiene el rol requerido, denegar acceso
  console.error(`❌ Role Guard: Usuario con rol ${currentUser.role} NO tiene acceso. Roles requeridos: ${allowedRoles.join(', ')}`);
