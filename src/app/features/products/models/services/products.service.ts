...
export class ProductsService {
  ...
  private transformFromBackendFormat(backendProduct: BackendProduct): Product {
    return {
      ...
      brandId: '1',
      lineId: backendProduct.lineaId.toString(),
  ...
  getProducts(filters?: ProductFilters): Observable<PaginatedResponse<Product>> {
    ...
    return this.http.get<BackendProduct[]>(`${this.apiUrl}/products`, { params }).pipe(
      map(backendProducts => {
  ...
  createBrand(data: { name: string; description?: string; logo?: File }): Observable<Brand> {
    ...
    return this.http.post<Brand>(`${this.apiUrl}/marcas`, formData).pipe(
      tap(brand => {
  ...
  getLines(brandId?: string): Observable<Line[]> {
    ...
    return this.http.get<Line[]>(`${this.apiUrl}/lineas`, { params }).pipe(
      tap(lines => {
  ...
  createLine(data: { name: string; description?: string; brandId: string }): Observable<Line> {
    ...
    return this.http.post<Line>(`${this.apiUrl}/lineas`, backendData).pipe(
      tap(line => {
